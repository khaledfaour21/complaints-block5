...
const ComplaintsDataContext = createContext<
  ComplaintsDataContextType | undefined
>(undefined);

export const ComplaintsDataProvider: React.FC<{
  ...
  const loadComplaints = useCallback(async () => {
    // Don't load data if no user or if we're on the login page
    ...
    try {
      ...
      setComplaints(data);
      setIsUsingDemoData(false); // Real data loaded successfully
    } catch (error) {
      console.error("Failed to load complaints:", error);
      // If API fails, getComplaints will return demo data, so set the flag
      const demoData = await api.getComplaints(user.role);
      setComplaints(demoData);
      setIsUsingDemoData(true); // Using demo data fallback
    } finally {
      setLoading(false);
    }
  ...
  const addComplaint = useCallback((newComplaint: Complaint) => {
    ...
    window.dispatchEvent(
      new CustomEvent("complaintAdded", {
        detail: newComplaint,
      })
    );
  }, []);
  ...
  const updateComplaint = useCallback(
    (id: string, updates: Partial<Complaint>) => {
      ...
      // Trigger real-time updates for all connected components
      window.dispatchEvent(
        new CustomEvent("complaintUpdated", {
          detail: { id, updates },
        })
      );
    },
  ...
  const deleteComplaint = useCallback((id: string) => {
    ...
    // Trigger real-time updates for all connected components
    window.dispatchEvent(
      new CustomEvent("complaintDeleted", {
        detail: id,
      })
    );
  }, []);
...
export const useComplaintsData = (): ComplaintsDataContextType => {
  ...
  if (!context) {
    throw new Error(
      "useComplaintsData must be used within a ComplaintsDataProvider"
    );
  }
  return context;
};
...
export const useComplaintsUpdates = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "complaintAdded",
      handleComplaintAdded as EventListener
    );
    window.addEventListener(
      "complaintUpdated",
      handleComplaintUpdated as EventListener
    );
    window.addEventListener(
      "complaintDeleted",
      handleComplaintDeleted as EventListener
    );

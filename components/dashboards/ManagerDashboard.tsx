...
import { User, Role, Complaint, ComplaintStatus, Importance } from '../../types';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, LineChart, Line, PieChart, Pie, Cell } from 'recharts';
import { UserGroupIcon, ChartBarIcon, UserPlusIcon, CogIcon, ShieldCheckIcon, ExclamationTriangleIcon, PhotoIcon, ArrowDownTrayIcon, FunnelIcon, PrinterIcon } from '@heroicons/react/24/outline';
...
export const ManagerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    loadUsers();
    api.getComplaints(Role.MANAGER).then(setSystemComplaints);
  ...
  const filteredData = useMemo(() => {
      return systemComplaints.filter(item => {
          ...
          return statusMatch && districtMatch && item.importance === Importance.HIGH;
      });
  ...
  const performanceData = React.useMemo(() => [
    { name: 'الأسبوع 1', nameEn: 'Week 1', resolved: Math.floor(Math.random() * 25) + 10, pending: Math.floor(Math.random() * 10) + 2 },
    { name: 'الأسبوع 2', nameEn: 'Week 2', resolved: Math.floor(Math.random() * 25) + 10, pending: Math.floor(Math.random() * 10) + 2 },
    { name: 'الأسبوع 3', nameEn: 'Week 3', resolved: Math.floor(Math.random() * 25) + 10, pending: Math.floor(Math.random() * 10) + 2 },
    { name: 'الأسبوع 4', nameEn: 'Week 4', resolved: Math.floor(Math.random() * 25) + 10, pending: Math.floor(Math.random() * 10) + 2 },
  ], []);

  const systemStats = React.useMemo(() => {
    const resolved = systemComplaints.filter(c => c.status === ComplaintStatus.COMPLETED).length;
    const pending = systemComplaints.filter(c => c.status === ComplaintStatus.IN_PROGRESS || c.status === ComplaintStatus.PENDING || c.status === ComplaintStatus.UNDER_REVIEW).length;
    const highImportance = systemComplaints.filter(c => c.importance === Importance.HIGH).length;
    const mediumImportance = systemComplaints.filter(c => c.importance === Importance.MEDIUM).length;
    const lowImportance = systemComplaints.filter(c => c.importance === Importance.LOW).length;
    const inProgress = systemComplaints.filter(c => c.status === ComplaintStatus.IN_PROGRESS).length;
    ...
    return {
      ...
      mediumImportance,
      lowImportance,
      resolutionRate: systemComplaints.length > 0 ? Math.round((resolved / systemComplaints.length) * 100) : 0,
    };
  }, [systemComplaints]);
  ...
  const handleSubmitUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUserFormData({ name: '', email: '', password: '', role: Role.MUKTAR, district: 'District 1' });
  ...
  return (
    <DashboardAccessControl dashboardType="manager">
      <StandardizedDashboardLayout
        ...
        subtitle={`Welcome back, ${user?.name}. Oversee operations across all districts.`}
        stats={systemStats}
      ...
      >
                          ...
                          <div className="bg-white dark:bg-gray-800 p-4 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg">
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-xs text-gray-500">
                                Percentage: {((data.value / (systemStats.completed + systemStats.pending)) * 100).toFixed(1)}%
                              </p>
                            </div>
                          </div>
                        );
                  ...
                  <table className="table table-zebra w-full">
                      ...
                      <tbody>
                          {table.getRowModel().rows.length === 0 ? (
                              <tr><td colSpan={12} className="text-center py-10 text-gray-400">No records found matching filters.</td></tr>
                          ) : (
        ...
        <div className="card bg-gradient-to-r from-brand-lightBg to-white dark:from-[#1a1a1a] dark:to-[#2a2a2a] shadow-xl">
          <div className="card-body">
            <h2 className="card-title font-cairo text-brand-primary mb-4">User Management</h2>
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
                ...
                <div className="flex gap-2">
                    <button className="btn btn-primary text-white gap-2" onClick={() => handleCreateUser(Role.MUKTAR)}><UserPlusIcon className="w-5 h-5" /> Create Muktar</button>
                    <button className="btn btn-neutral text-white gap-2" onClick={() => handleCreateUser(Role.ADMIN)}><UserPlusIcon className="w-5 h-5" /> Create Admin</button>
                </div>
            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    {selectedMuktar ? (
                        <div className="animate-fade-in">
                             <h3 className="text-2xl font-bold mb-4 font-cairo">Performance: <span className="text-brand-accent">{selectedMuktar.name}</span></h3>
                                   ...
                                   <div className="h-64">
                                       <ResponsiveContainer width="100%" height="100%">
                                           <LineChart data={performanceData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                               <defs>
                                                 <linearGradient id="resolvedGradient" x1="0" y1="0" x2="1" y2="0">
                                                     ...
                                                     return (
                                                       <div className="bg-white dark:bg-gray-800 p-3 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg">
                                                         ...
                                                         <p className="text-sm text-orange-600">
                                                           Pending: <span className="font-bold">{payload[0].value}</span>
                                                         </p>
                                                       </div>
                                                     );
                                                   }
      ...
      {showCreateUserModal && (
          <div className="modal modal-open">
              <div className="modal-box">
                  <h3 className="font-bold text-lg mb-4">Create New {userFormData.role}</h3>
                  <form onSubmit={handleSubmitUser}>
                      ...
                      {userFormData.role === Role.MUKTAR && (
                          <div className="form-control mb-4">
                              ...
                              <select
                                  ...
                                  onChange={(e) => setUserFormData(prev => ({ ...prev, district: e.target.value }))}
                              >
                                  <option value="District 1">District 1</option>
                                  <option value="District 2">District 2</option>
                                  <option value="District 3">District 3</option>
                      ...
                      <div className="modal-action">
                          <button type="button" className="btn" onClick={() => setShowCreateUserModal(false)}>Cancel</button>

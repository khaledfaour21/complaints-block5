...
import { Complaint, ComplaintStatus, Role, Importance, User } from '../../types';
import { useLangStore, useAuthStore } from '../../store';
import { TableSkeleton } from '../shared/LoadingStates';
import { ArrowDownTrayIcon, FunnelIcon, PrinterIcon, ChartBarIcon, UsersIcon, ExclamationTriangleIcon, CheckCircleIcon, PhotoIcon, CogIcon, ShieldCheckIcon } from '@heroicons/react/24/outline';
...
export const AdminDashboard: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
      ...
      // Filter by selected admin or user role
      if (user?.role === Role.MANAGER && selectedAdmin) {
        filtered = filtered.filter(item => item.assignedAdminId === selectedAdmin.id);
      } else if (user?.role === Role.ADMIN) {
        filtered = filtered.filter(item => item.assignedAdminId === user.id);
      }
      // For ADMIN role, show their own assigned complaints

      return filtered.filter(item => {
          const statusMatch = statusFilter ? item.status === statusFilter : true;
          const districtMatch = districtFilter ? item.district === districtFilter : true;
          const importanceMatch = importanceFilter ? item.importance === importanceFilter : true;
          return statusMatch && districtMatch && importanceMatch;
      });
  }, [complaints, user, selectedAdmin, statusFilter, districtFilter, importanceFilter]);
                ...
                <button 
                    onClick={() => {
                        if (confirm('Are you sure you want to delete this complaint?')) {
                            api.deleteComplaint(info.row.original.id);
                            setData(prev => prev.filter(p => p.id !== info.row.original.id));
                        }
                    }}
  ...
  const stats = useMemo(() => {
    const relevantComplaints = user?.role === Role.MANAGER && selectedAdmin
    ...
    return {
      ...
      completed: relevantComplaints.filter(c => c.status === ComplaintStatus.COMPLETED).length,
  ...
  const districtData = useMemo(() => {
    ...
    return districts.map(d => ({
      name: d.name,
      nameAr: d.nameAr,
      complaints: relevantComplaints.filter(c => c.district === d.name).length,
      color: d.color,
      fill: d.color
    }));
  }, [complaints, user, selectedAdmin]);
  ...
  return (
    <DashboardAccessControl dashboardType="admin">
      <StandardizedDashboardLayout
        ...
        subtitle={user?.role === Role.MANAGER ? `Select an Admin to view their dashboard` : "Monitor and manage the entire complaint system"}
      ...
      >
                ...
                <div className="flex flex-col md:flex-row gap-4 items-end">
                  <div className="form-control w-full md:w-1/2">
                    <label className="label">
                      <span className="label-text text-white">Choose Admin to View Dashboard</span>
                    </label>
                    <select
                  ...
                  {selectedAdmin && (
                    <div className="stats bg-white/20 backdrop-blur-sm">
                      <div className="stat place-items-center">
                        <div className="stat-title text-white">Selected Admin</div>
                        <div className="stat-value text-white text-lg">{selectedAdmin.name}</div>
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-white dark:bg-gray-800 p-3 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg">
                              <p className="font-semibold text-brand-primary">{data.name}</p>
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-4 bg-base-100 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex flex-wrap gap-4 w-full md:w-auto">
              ...
              <select className="select select-bordered select-sm" onChange={e => setImportanceFilter(e.target.value)}>

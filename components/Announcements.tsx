...
import { MegaphoneIcon, PlusIcon, TrashIcon, MapPinIcon } from '@heroicons/react/24/outline';
...
export const Announcements: React.FC = () => {
  ...
  const canEdit = user && (user.role === Role.ADMIN || user.role === Role.MANAGER);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center">
        ...
        {canEdit && (
            <button onClick={() => setModalOpen(true)} className="btn btn-primary text-white gap-2">
                <PlusIcon className="w-5 h-5" /> {t('ann.create')}
            </button>
        )}
      </div>

      {loading ? <div className="flex justify-center p-10"><span className="loading loading-bars loading-lg"></span></div> : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {announcements.map((ann) => (
                <div key={ann.id} className={`card bg-base-100 shadow-xl transition-all hover:shadow-2xl ${ann.isSticky ? 'border-2 border-brand-accent' : ''}`}>
                    <div className="card-body">
                        <div className="flex justify-between items-start">
                            <div className="badge badge-outline">{ann.category}</div>
                            {canEdit && <button onClick={() => handleDelete(ann.id)} className="btn btn-ghost btn-xs text-error"><TrashIcon className="w-4 h-4"/></button>}
                        </div>
                        <h2 className="card-title text-xl mt-2">{ann.title}</h2>
                        {ann.isSticky && <span className="text-xs font-bold text-brand-accent uppercase tracking-widest flex items-center gap-1"><MapPinIcon className="w-3 h-3"/> Pinned</span>}
                        <p className="text-gray-600 dark:text-gray-300 mt-2">{ann.description}</p>
            ...
            <div className="modal-box">
                ...
                <form onSubmit={handleCreate} className="space-y-4">
                    ...
                    <label className="label cursor-pointer justify-start gap-4 border rounded p-2">
                        <input name="sticky" type="checkbox" className="checkbox checkbox-primary" />
                    ...
                    <div className="modal-action">
                        ...
                        <button type="submit" className="btn btn-primary text-white">Post</button>
                    </div>
                </form>
            </div>
        </div>

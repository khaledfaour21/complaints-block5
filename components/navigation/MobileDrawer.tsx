  ...
  return (
    <ul className="menu p-4 w-80 min-h-full bg-base-100 text-base-content border-r dark:border-gray-700">
      <li className="mb-6 px-4">
        <h2 className="text-2xl font-bold text-brand-primary font-cairo">
          Fifth Block
        </h2>
      ...
      {user ? (
        <>
          ...
          <li>
            <Link to="/settings" onClick={onClose}>
              <Cog6ToothIcon className="w-5 h-5" /> {t("nav.settings")}
            </Link>
          </li>
          <li onClick={handleLogout} className="mt-4">
            <a className="text-error bg-red-50 dark:bg-red-900/20">
        ...
        <li>
          <Link
            to="/login"
            onClick={onClose}
            className="btn btn-outline btn-sm btn-block text-center justify-center mt-4"
          >
            {t("nav.login")}
          </Link>
        </li>

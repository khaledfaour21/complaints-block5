...
import { MegaphoneIcon, PlusCircleIcon, DocumentMagnifyingGlassIcon, TrophyIcon, ArrowRightIcon } from '@heroicons/react/24/outline';

export const Home: React.FC = () => {
  const { t, lang } = useLangStore();
  const [stickyAnnouncements, setStickyAnnouncements] = useState<Announcement[]>([]);
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-12">
        ...
        {stickyAnnouncements.length > 0 && (
            <div className="space-y-2">
                {stickyAnnouncements.map(ann => (
                    <div key={ann.id} className="alert bg-brand-accent text-brand-secondary shadow-lg border-l-8 border-brand-secondary rounded-lg">
                        <MegaphoneIcon className="w-6 h-6 animate-pulse" />
                        <div>
                            <h3 className="font-bold uppercase tracking-wide text-xs">Announcement</h3>
                            <div className="font-bold text-lg">{ann.title}</div>
                            <div className="text-sm opacity-90">{ann.description}</div>
                        </div>
                        <Link to="/announcements" className="btn btn-sm btn-ghost">View All</Link>
                    </div>
                ))}
            </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Link to="/submit" className="card bg-base-100 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-transparent hover:border-brand-accent">
            ...
            <Link to="/track" className="card bg-base-100 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-transparent hover:border-brand-accent">
                <div className="card-body items-center text-center">
                    <div className="w-16 h-16 bg-brand-lightBg rounded-full flex items-center justify-center text-brand-primary mb-4">
            ...
            <Link to="/achievements" className="card bg-base-100 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border border-transparent hover:border-brand-accent">
                <div className="card-body items-center text-center">
                    <div className="w-16 h-16 bg-brand-lightBg rounded-full flex items-center justify-center text-brand-primary mb-4">

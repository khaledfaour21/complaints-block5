...
export const EnhancedComplaintTable: React.FC<EnhancedComplaintTableProps> = ({
  ...
  const handleUpdateImportance = (complaintId: string, newImportance: Importance) => {
      ...
      columnHelper.accessor('assignedMuktarId', {
        ...
        cell: ({ row }) => (
          <div className="flex flex-col gap-1">
            <span className={`badge ${getAssignedRoleBadgeClass(row.original)} badge-sm`}>
      ...
      cell: ({ row }) => (
        <div className="w-full">
          <select
            ...
            onChange={(e) => handleUpdateImportance(row.original.id, e.target.value as Importance)}
          >
      ...
      cell: ({ row }) => (
        <div className="w-full">
          <select
            ...
            onChange={(e) => handleUpdateStatus(row.original.id, e.target.value as ComplaintStatus)}
          >
      ...
      cell: ({ row }) => (
        <div className="flex justify-center">
          <input
            ...
            onChange={(e) => handleTogglePin(row.original.id, e.target.checked)}
          />
        </div>
      ),
      size: 80,
  ...
  ], [showAssignedRole, isManagerView, isAdminView, onUpdateComplaint, onDeleteComplaint, onViewDetails]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <select
            value={(table.getColumn('importance')?.getFilterValue() as string) ?? ''}
            onChange={(e) =>
              table.getColumn('importance')?.setFilterValue(e.target.value || undefined)
            }
            className="select select-bordered select-sm"
          >
        ...
        <div className="card-body p-0">
              ...
              <tbody>
                {table.getRowModel().rows.length === 0 ? (
                  ...
                  table.getRowModel().rows.map((row) => (
                    <tr key={row.id} className="hover:bg-brand-lightBg/50 transition-colors">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-gray-200">
            <span className="text-sm text-gray-500">
              Showing {table.getRowModel().rows.length} of {complaints.length} complaints
            </span>
            <div className="flex items-center gap-2">
              <button
